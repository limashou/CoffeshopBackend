...
public class OfferService {
    ...
    public void delete(Long id) {
     ...
     public List<OfferRequest> getAllbyCoffeeshop(Long id) {
         ...
         return offerList.stream()
                 .map(offer -> {
                     Map<Menu, Integer> itemsList = offer.getItems();
    ...
    public SpecialOffer save(OfferResponse offerDTO){
        if(offerDTO.getCoffeeshopId() == null || offerDTO.getItemsWithQuantity() == null) {
            throw new AppRuntimeException("Parameter coffeeshop_id or items is not found in request..!!", HttpStatus.NO_CONTENT);
        }
        SpecialOffer specialOffer = new SpecialOffer(offerDTO.getId(),offerDTO.getName(),
                ...
                offerDTO.getItemsWithQuantity().entrySet().stream()
                .collect(Collectors.toMap(entry -> menuService.menuRepository.findById(entry.getKey()).orElse(null),
                        Map.Entry::getValue)),
                calculateTotalPrice(offerDTO.getItemsWithQuantity()));
